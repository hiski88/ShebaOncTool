...
# Streamlit Cloud may return from Google in a fresh session. In that case the
# session-scoped copy of google_oauth_state is not reliable. The callback still
# validates the signed, time-limited state value itself, so discard only the
# fragile session comparison when an actual OAuth code is present.
